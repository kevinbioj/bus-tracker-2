...
export function VehicleLive({ vehicle }: Readonly<VehicleLiveProps>) {
	...
	return (
		<Link
			className="relative isolate block border border-border rounded-md shadow-lg min-w-64 h-32 lg:h-auto lg:aspect-square mt-3 overflow-hidden [will-change:transform]"
			to="/"
		...
		>
			...
			<MapContainer
				...
				ref={mapRef}
			>
				<p className="absolute bg-white font-bold top-0.5 left-0.5 px-2 pt-0.5 rounded-md text-black uppercase z-400">
					<CircleIcon className="align-text-bottom animate-pulse fill-red-500 stroke-none size-4 inline" />{" "}
					{m.vehicle_details_click_live()}
				</p>
