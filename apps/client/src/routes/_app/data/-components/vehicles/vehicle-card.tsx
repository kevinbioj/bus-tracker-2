...
export function VehicleCard({ vehicle }: Readonly<{ vehicle: Vehicle }>) {
	...
	return (
		<Link
			className={`border border-border flex flex-col sm:flex-row py-1 px-2 rounded-md hover:brightness-90 active:not-aria-[haspopup]:translate-y-px transition-[filter,transform] ${
		...
		>
			...
			<div
				className="border-t sm:border-l border-black dark:border-white mx-2 my-1"
				style={{ borderColor: line?.textColor ?? undefined }}
			/>
