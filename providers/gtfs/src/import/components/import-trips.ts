...
export async function importTrips(
  ...
  await readCsv<TripRecord>(join(gtfsDirectory, "trips.txt"), (tripRecord) => {
    ...
    if (typeof route === "undefined") {
      throw new Error(
        `Unknown route with id '${tripRecord.route_id}' for trip '${tripRecord.trip_id}'.`
      );
    }
    ...
    if (typeof service === "undefined") {
      throw new Error(
        `Unknown service with id '${tripRecord.service_id}' for trip '${tripRecord.trip_id}'.`
      );
    }
    ...
    const trip = new Trip(
      ...
      typeof tripRecord.shape_id !== "undefined"
        ? shapes.get(tripRecord.shape_id)
        : undefined
    );
  ...
  await readCsv<StopTimeRecord>(
    join(gtfsDirectory, "stop_times.txt"),
    (stopTimeRecord) => {
      const tripId =
        mapTripId?.(stopTimeRecord.trip_id) ?? stopTimeRecord.trip_id;
      ...
      const stopTime = new StopTime(
        ...
        createPlainTime(
          `${(arrivalHours % 24)
            .toString()
            .padStart(2, "0")}:${stopTimeRecord.arrival_time.slice(3)}`
        ),
        ...
        mismatchingTimes
          ? createPlainTime(
              `${(departureHours % 24)
                .toString()
                .padStart(2, "0")}:${stopTimeRecord.departure_time.slice(3)}`
            )
          : undefined,
        mismatchingTimes ? Math.floor(departureHours / 24) : undefined,
        typeof stopTimeRecord.shape_dist_traveled !== "undefined"
          ? +stopTimeRecord.shape_dist_traveled
          : undefined
      );
