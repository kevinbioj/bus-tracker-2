...
export class Journey {
  ...
  guessPosition(at: Temporal.Instant): VehicleJourneyPosition {
    ...
    const monitoredCall =
      nextCallIndex === 0 ? nextCall : calls.at(nextCallIndex - 1)!;
    ...
    const nextArrivalTime = (
      nextCall.expectedArrivalTime ?? nextCall.aimedArrivalTime
    ).epochSeconds;
    const inBetweenTime = nextArrivalTime - lastDepartureTime;
    const percentTraveled =
      (at.epochSeconds - lastDepartureTime) / inBetweenTime;
    const estimatedDistanceTraveled =
      monitoredCallDistanceTraveled +
      (nextCallDistanceTraveled - monitoredCallDistanceTraveled) *
        percentTraveled;

    const currentShapePoint =
      this.trip.shape.points.findLast(
        (point) => estimatedDistanceTraveled >= point.distanceTraveled
      ) ?? this.trip.shape.points.at(-1)!;
    const nextShapePoint =
      this.trip.shape.points.at(
        this.trip.shape.points.indexOf(currentShapePoint) + 1
      ) ?? this.trip.shape.points.at(-1)!;
    const ratio =
    ...
    return {
      latitude:
        ...
        (nextShapePoint.latitude - currentShapePoint.latitude) * ratio,
      longitude:
        currentShapePoint.longitude +
        (nextShapePoint.longitude - currentShapePoint.longitude) * ratio,
      atStop: false,
      type: "COMPUTED",
      recordedAt: at
  ...
  hasRealtime() {
    return this.calls.some(
      (call) =>
        typeof call.expectedArrivalTime !== "undefined" ||
        typeof call.expectedDepartureTime !== "undefined"
    );
  }
  ...
  updateJourney(stopTimeUpdates: StopTimeUpdate[]) {
    ...
    for (const call of this.calls) {
      const timeUpdate = stopTimeUpdates?.find(
        (stu) =>
          stu.stopSequence === call.sequence || stu.stopId === call.stop.id
      );
      ...
      if (typeof departureEvent?.time === "number") {
        departureDelay =
          departureEvent.time - call.aimedDepartureTime.epochSeconds;
  ...
  private static getJourneyPositionAt(
    call: JourneyCall
