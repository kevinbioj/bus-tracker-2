...
export class Source {
  ...
  constructor(readonly id: string, readonly options: SourceOptions) {}
  ...
  async importGtfs(updating = false) {
    ...
    try {
      ...
      updateLog(
        "%s 2/3 ► Loading GTFS resource contents into memory...",
        sourceId
      );
      const gtfs: Gtfs = {
      ...
      if (typeof this.options.excludeScheduled !== "boolean") {
        ...
        for (const trip of gtfs.trips.values()) {
          ...
          for (const journey of journeys) {
            ...
            if (
              now.epochSeconds >
              journey.calls.at(-1)!.aimedDepartureTime.epochSeconds
            )
              continue;
        ...
        gtfs.journeys.sort((a, b) =>
          Temporal.ZonedDateTime.compare(
            a.calls[0]!.aimedArrivalTime,
            b.calls[0]!.aimedArrivalTime
          )
        );
      }
  ...
  async updateGtfs() {
    const sourceId = padSourceId(this.id);
    const updateLog = console.draft(
      "%s ► Checking GTFS resource staleness.",
      sourceId
    );

    if (typeof this.gtfs === "undefined") {
      updateLog(
        "%s ℹ Resource has not loaded yet (error?), performing a load attempt.",
        sourceId
      );
    ...
    if (this.gtfs.lastModified === null && this.gtfs.etag === null) {
      const delta = Temporal.Now.instant()
        .since(this.gtfs.importedAt)
        .total("minutes");
      if (delta >= 60) {
        updateLog(
          "%s ℹ Current resource is older than 60 minutes (no staleness data): updating resource.",
          sourceId
        );
        return this.importGtfs(true);
      }
      updateLog(
        "%s ℹ Current resource is fresh enough (no staleness data).",
        sourceId
      );
      return;
    }
    ...
    try {
      updateLog(
        "%s ► Fetching resource staleness at '%s'.",
        sourceId,
      ...
      if (
        this.gtfs.lastModified !== staleness.lastModified ||
        this.gtfs.etag !== staleness.etag
      ) {
        updateLog(
          "%s ℹ Fetched staleness is different than current: updating resource.",
          sourceId
        );
        return this.importGtfs(true);
      }
      updateLog(
        "%s ℹ Fetched staleness matches current staleness: keeping current resource.",
        sourceId
      );
    } catch {
      const delta = Temporal.Now.instant()
      ...
      updateLog(
        "%s ⚠ Failed to fetch resource staleness, but current resource looks fresh enough.",
        sourceId
      );
    } finally {
      console.log();
  ...
  computeNextJourneys() {
    ...
    const updateLog = console.draft(
      "%s ► Computing journeys for date '%s'.",
      sourceId,
      date
    );
    const watch = createStopWatch();
    ...
    updateLog(
      "%s ✓ Computed %d journeys for date '%s' in %dms.",
      sourceId,
      computedJourneys,
      date,
      watch.total()
