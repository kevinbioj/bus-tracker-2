...
const sources = [
  {
    ...
    getOperatorRef: (journey, vehicle) => {
      if (journey?.trip.route.id === "06" || journey?.trip.route.id === "89")
        return "TNI";
      if (
    ...
    getDestination: (journey, vehicle) =>
      vehicle?.label ?? journey.calls.at(-1)?.stop.name ?? "Spécial",
  },
  {
    id: "tae",
    ...
    mapTripUpdate: (tripUpdate) => {
      ...
      if (
        tripUpdate.stopTimeUpdate?.some(({ arrival }) => arrival?.delay > 5400)
      )
        return;
      return tripUpdate;
    },
    ...
    getAheadTime: (journey) =>
      journey?.calls.some(
        (c) => !!(c.expectedArrivalTime ?? c.expectedDepartureTime)
      )
        ? 15 * 60
        : 0,
    getNetworkRef: () => "ASTUCE",
  ...
  },
  {
    id: "tni",
    staticResourceHref:
      "https://www.data.gouv.fr/fr/datasets/r/e39d7fe1-8c0c-4273-9236-d7c458add7a0",
    realtimeResourceHrefs: [
    ...
    getNetworkRef: (journey) => {
      if (
        ...
        [journey.calls.at(0), journey.calls.at(-1)].some(
          (call) => call.stop.name === "Caudebec - Quai"
        )
      ) {
        return "NOMAD-CAR";
      }
    ...
    getDestination: (journey) =>
      `${journey.calls.at(0)?.stop.name} > ${journey.calls.at(-1)?.stop.name}`,
    getOperatorRef: () => "TNI",
  },
  {
    id: "hanga",
    staticResourceHref:
      "https://exs.tcar.cityway.fr/gtfs.aspx?key=OPENDATA&operatorCode=ASTUCE&companyCode=ASTUCE:004",
    realtimeResourceHrefs: [],
    getNetworkRef: () => "ASTUCE",
    getOperatorRef: (journey) =>
      ["204", "214"].includes(journey?.trip.route.id) ? "TNI" : "HANGA",
